...
void ordenacaoTopologica(TipoGrafo *grafo){
    PLL kar = buscaProfundidadeTopologica(grafo);
    printf("\nORDENAÇÃO\n");
    if(kar->primL != NULL){
        PV flash = kar->primL;
	printf("\n")
...
*/
    ...
    PLL lista = (PLL) malloc(sizeof(ListaLigada));
/*
    Inicializa a lista.
*/
    lista->primL = NULL;
/*
    Faz a busca em profundidade para cada vértice caso ele não tenha sido visitado.
*/
    //printf("iniciei a lista");
    for(i=1;i<=grafo->numVertices;i++){
        if(cor[i] == branco)    visitaBPTop(i,grafo,lista,&tempo,cor,tdesc,tterm,antecessor);
    }
    return lista;
}
...
void visitaBPTop(int v,TipoGrafo *grafo,PLL lista,int *tempo,int *cor,int *tdesc, int *tterm, int *antecessor){
    //printf("show,vertice: %2i\n",v);
    cor[v] = cinza;  //Passou pelo vértice
    tdesc[v] = ++(*tempo);  //Tempo de descoberta
    TipoApontador p = grafo->listaAdj[v]; //Pega o vértice adjacente de v.
    while(p){
        if(cor[p->vdest] == branco){  //Procura vértices em branco para explorar
            antecessor[p->vdest] = v;    //Guarda quem foi seu antecessor
            visitaBPTop(p->vdest,grafo,lista,tempo,cor,tdesc,tterm,antecessor); //Chama o recursivo para a função, USE O P->VDEST para ir para o vértice seguinte!!!!!!
        }
        p = p->prox;
    }
    tterm[v] = ++(*tempo); //Depois de ter passado pelos outros, esse termina
    cor[v] = preto; //Recebe preto por ter terminado
    PV ver = (PV) malloc(sizeof(Vertice));
    ver->vert = v;
    ver->proxVert = lista->primL;
    lista->primL = ver;
}
